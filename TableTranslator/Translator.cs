...
using TableTranslatorEx.Engines;
...
using TableTranslatorEx.Stores;

namespace TableTranslatorEx
{
    /// <summary>
    /// Main entry point for TableTranslatorEx interacting with profiles and translations
    /// </summary>
    public static class Translator
    {
        ...
        public static void Initialize()
        {
            if (IsInitialized)
            {
                throw new TableTranslatorException("Table translator has already been initialized");
            }
        ...
        public static void ApplyUpdates()
        {
            if (!IsInitialized)
            {
                throw new TableTranslatorException("You must initialize the translator before calling ApplyUpdates().");
            }
        ...
        {
            if (translationProfiles == null)
            {
                throw new ArgumentNullException(nameof(translationProfiles));
            }

            var list = translationProfiles as TranslationProfile[] ?? translationProfiles.ToArray();
            if (list.Any(x => x == null))
            {
                throw new ArgumentException("Argument translationProfiles contained a item that was null.");
            }
        ...
        private static void PreTranslateValidation()
        {
            if (!IsInitialized)
            {
                throw new TableTranslatorException("You must initialize the translator by calling 'Translator.Initialize()' before perfoming a translation.");
            }
        }
    }
}